...
Query::Ptr ManageTableQueryBuilder::tryExtractQuery
    ...
    if (query.token.size() == 2) {
        if (query.token.front() == "LOAD"){
            ...
            //std::cout << "file token: " << query.token[1] << std::endl;
            auto tableName = db.getFileTableName(query.token[1]);
           // std::cout << "tname: " << tableName << std::endl;
            auto ptr = new LoadTableQuery(tableName, query.token[1]);
            return std::unique_ptr<LoadTableQuery>(ptr);
        }
        if (query.token.front() == "DROP") {
            auto ptr = new DropTableQuery(query.token[1]);
            return std::unique_ptr<DropTableQuery>(ptr);
        }
        if (query.token.front() == "TRUNCATE") {
            auto ptr = new TruncateTableQuery(query.token[1]);
            return std::unique_ptr<TruncateTableQuery>(ptr);
        }
    }
    if (query.token.size() == 3) {
        // TODO: will have problems?
        if (query.token.front() == "DUMP") {
            auto &db = Database::getInstance();
            db.updateFileTableName(query.token[2], query.token[1]);
            auto ptr = new DumpTableQuery(query.token[1], query.token[2]);
            return std::unique_ptr<DumpTableQuery>(ptr);
        }
        if (query.token.front() == "COPYTABLE") {
            auto ptr = new CopyTableQuery(query.token[1], query.token[2]);
            return std::unique_ptr<CopyTableQuery>(ptr);
        }
    }
    if (query.token.size() == 4) {
        if (query.token.front() == "LISTEN") {
            // return std::make_unique<NopQuery>(); // Not implemented
            auto ptr = new ListenQuery(query.token[2]);
            return std::unique_ptr<ListenQuery>(ptr);
        }
    }
    return this->nextBuilder->tryExtractQuery(query);
}
...
Query::Ptr DebugQueryBuilder::tryExtractQuery(TokenizedQueryString &query) {
    if (query.token.size() == 1) {
        ...
        if (query.token.front() == "QUIT") {
            auto ptr = new QuitQuery();
            return std::unique_ptr<QuitQuery>(ptr);
        }
    }
    if (query.token.size() == 2) {
...
Query::Ptr ComplexQueryBuilder::tryExtractQuery(TokenizedQueryString &query) {
    ...
    if (operation == "INSERT") {
        InsertQuery* ptr = new InsertQuery(this->targetTable, this->operandToken, this->conditionToken);
        // naming all the ids
        ptr->id = 1;
        return std::unique_ptr<InsertQuery>(ptr);
    }
    // TODO: Modify the id of the following operations
    if (operation == "UPDATE") {
